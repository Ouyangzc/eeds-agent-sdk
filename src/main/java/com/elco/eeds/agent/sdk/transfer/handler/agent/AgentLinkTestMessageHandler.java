 ...
 */
public class AgentLinkTestMessageHandler extends IReceiverMessageHandler {
  ...
  public void handleRecData(String topic, String recData) {
    try {
      ...
      AgentLinkTestMessage agentLinkTestMessage = JSON.parseObject(recData,
          AgentLinkTestMessage.class);
      AgentLinkTestMessage rspMessage = AgentLinkTestMessage.getRspMessage(
          agentLinkTestMessage.getData().getPkUser(), agentLinkTestMessage.getData().getSocketId());
      String rspTopic = ConstantTopic.TOPIC_AGENT_LINK_TEST_RSP.replace("{agentId}",
          agent.getAgentBaseInfo().getAgentId());
      MqPluginUtils.sendAgentLinkTestMsg(rspTopic, JSONUtil.toJsonStr(rspMessage));
    } catch (Exception e) {
