...
public class HttpClientUtil {
  ...
  public static String httpPost(String url, String token, String content) throws Exception {
    ...
    HttpClient client = HttpClientBuilder.create().setRetryHandler(new HttpRetryHandler()).build();
    try {
      ...
      logger.error("调用server接口异常, 请求地址为：{}，形参为：{},异常为:{}", url,
  ...
  public static String httpsPost(String url, String token, String content) throws Exception {
    try {
      ...
      se.setContentEncoding(new BasicHeader(HTTP.CONTENT_TYPE, APPLICATION_JSON));
      httpPost.setEntity(se);
      HttpResponse httpResponse = HttpClients.custom().setRetryHandler(new HttpRetryHandler())
          .setSSLSocketFactory(socketFactory).build().execute(httpPost);
      // 得到应答的字符串，这也是一个 JSON 格式保存的数据
      String resp = EntityUtils.toString(httpResponse.getEntity(), CHARSET_UTF_8);
      logger.debug("response: {}", resp);
      return resp;
